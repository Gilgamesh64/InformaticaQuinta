...
public class Junction {
    ...
    @SuppressWarnings("unchecked")
    public static Relation apply(Relation one, Relation two, ArrayList<String> junctionFieldR1, ArrayList<String> junctionFieldR2){
        String csv = "";
        for (String string : junctionFieldR1) {
            if(!one.getHeader().contains(string)) return new Relation(csv);
        }
        for (String string : junctionFieldR2) {
            if(!two.getHeader().contains(string)) return new Relation(csv);
        }

        csv += one.getHeader().toString().replaceAll("]", "").replaceAll("\\[", "").replaceAll(" ", "") + ",";
        for (String string : two.getHeader()) {
            if(junctionFieldR2.contains(string)) continue;
            csv += string + ",";
        }
        csv = csv.substring(0, csv.length()-1) + "\n";
